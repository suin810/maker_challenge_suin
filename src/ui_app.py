...
def create_app():
    ...
    with gr.Blocks(
        ...
        css="""
        #modal_overlay { position: fixed; inset: 0; background: rgba(0,0,0,0.45); z-index: 9999; display: flex; align-items: center; justify-content: center; }
        #modal_box { background: white; padding: 24px; border-radius: 10px; box-shadow: 0 10px 30px rgba(0,0,0,0.2); max-width: 560px; width: calc(100% - 32px); }
        #modal_box h3 { margin-top: 0; }
        """,
    ) as demo:
        ...
        # ----------------------
        # 저장 진행/완료 모달 (커스텀 오버레이)
        # ----------------------
        with gr.Group(visible=False, elem_id="modal_overlay") as modal_overlay:
            with gr.Column(elem_id="modal_box"):
                modal_message = gr.Markdown("저장 중입니다… 잠시만 기다려주세요.")
                modal_confirm_btn = gr.Button("확인", visible=False, variant="primary")
        ...
        # 저장 로딩 표시 (1단계): 모달 열고 로딩 메시지 출력
        def show_loading_modal():
            return gr.update(visible=True), gr.update(value="### 저장 중입니다…\n잠시만 기다려주세요."), gr.update(visible=False)

        loading_event = save_btn.click(
            fn=show_loading_modal,
            outputs=[modal_overlay, modal_message, modal_confirm_btn],
        )

        # 저장 실행 (2단계): DB 저장 후 완료 메시지로 변경 및 확인 버튼 노출
        def do_save_and_update_modal(name: str, date_text: str, participants_rows: List[List[str]], segs: List[Dict], apath: str, full_text: str):
            try:
                # 기존 do_save 로직을 재사용 (페이지 전환은 여기서 하지 않음)
                # 참가자 매핑 파싱 및 스피커 이름 치환
                def _parse_participants_from_df(rows: List[List[str]]) -> Dict[str, str]:
                    mapping: Dict[str, str] = {}
                    for row in (rows or []):
        ...
        loading_event.then(
            fn=do_save_and_update_modal,
            inputs=[save_name, save_date, participants_df, last_segments, last_audio_path, last_transcript_text],
            outputs=[modal_message, modal_confirm_btn],
        )

        # 모달 확인 버튼: 목록으로 전환 + 목록 갱신 + 모달/저장 패널 닫기
        def confirm_and_go_list():
            return (
                gr.update(visible=True),   # show list page
                gr.update(visible=False),  # hide transcribe page
                "list",
                gr.update(choices=_load_meetings(), value=None),
                gr.update(visible=False),  # hide modal
                gr.update(visible=False),  # hide save panel
            )

        modal_confirm_btn.click(
            fn=confirm_and_go_list,
            outputs=[page_list, page_transcribe, page_state, meetings_dropdown, modal_overlay, save_panel],
        )
